...
def train(args, bert_field, model):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        logger.debug('train loss:%f' % loss_epoch.item())
        print('train result', train_right, train_total, 1.0 * train_right / train_total)
        logger.debug(('train result', train_right, train_total, 1.0 * train_right / train_total))
        ...
        print(datetime.now())
        print('*' * 20)
        logger.debug("epoch:%d\t" % epoch + "dev_Accuracy-----------------------%d/%d=%f\n" % (right, total, precision))
        end_time = datetime.now()
        logger.debug('dev epoch %d time span:%s' % (epoch, end_time - start_time))

        if precision > best_precision:
            best_precision = precision
            iters_left = patience
            print("epoch %d saved\n" % epoch)
            logger.debug("epoch %d saved\n" % epoch)
...
if __name__ == "__main__":
    ...
    args.model=MODEL
    args.train_file='data/'+SCALE+'/train.json'
    args.valid_file='data/'+SCALE+'/valid.json'
